...
class Command(object):
    def __init__(self, id, direction, speed):
        ...
        :param id:
            The elevator id.  This is returned in the state where all elevators
              are identified.
        ...
        :param direction:
            1 to indicate up, -1 to indicate down. Even when stationary an
            elevator has a direction.  Passengers wishing to go up will not
            board an elevator with a down indicator.
        :type direction:
            `int`
    ...
    def __str__(self):
        return "set car {} (direction={}, speed={})".format(self.id,
                                                            self.direction,
                                                            self.speed)
...
class BoxLift(object):
    ...
    def __init__(self, bot_name, plan, email, registration_id, event_name='',
        ...
        :type plan:
            `basestring`
        :param email:
            A contact email. This helps us contact you if there are problems or
            to inform you if you have one of the top 10 finishers. Prizes will
            be distributed in the Expo Hall on Sunday breakfast and lunch.
        :type email:
            `basestring`
        :param registration_id:
            A unique id for the event. Required for prize pickup. e.g. your
            Pycon 2015 registration number
        ...
        :param sandbox_mode:
            ...
            competition. Useful for testing and for event
            organizers who want to play, but don't want to be on the leader
            board. Tokens will not expire, allowing you to step through code
            slowly.
        :type sandbox_mode:
            `bool`
        """
    ...
    def _get_world_state(self, initialization_data):
        """Initialize and gets the state of the world without sending any
        commands to advance the clock"""
    ...
    def send_commands(self, commands=None):
        ...
        :param commands:
            list of commands to elevators. If no commands are sent the clock
            does not advance.
        :type commands:
            `iterable` of `Command`
        :return:
        ...
        for command in commands:
            command_list[command.id] = {'speed': command.speed, 'direction':
                                        command.direction}
